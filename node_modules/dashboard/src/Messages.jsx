...
export default function Messages() {
  ...
  return (
    <div className="bg-[#0b1325] text-[#dbe2fb] min-h-screen flex font-['Inter',sans-serif]">
      <Sidebar />

      {/* Main Content Canvas */}
      <main className="md:ml-[260px] flex-1 flex h-screen overflow-hidden bg-[#0b1325]">
        {/* Left Column: Conversation List */}
        <section className="w-80 border-r border-[#3c4a45]/10 bg-[#131b2d] flex flex-col">
          <div className="p-6">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-0.5">
                      <span className={`font-${activeConversationId === conv.id ? 'semibold' : 'medium'} text-sm truncate ${activeConversationId === conv.id ? 'text-[#44e5c2]' : 'text-[#dbe2fb]'}`}>
                        {conv.participantName || 'User'}
                      </span>
                      <span className="text-[10px] text-[#bacac3] font-['JetBrains_Mono',monospace]">
        ...
        <section className="flex-1 flex flex-col relative w-full h-full">
          {activeConversation ? (
            <>
              <header className="h-20 bg-[#0b1325]/40 backdrop-blur-md flex items-center justify-between px-8 border-b border-[#3c4a45]/5 z-10 flex-shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-[#dbe2fb]">{activeConversation.participantName}</h3>
                    {activeConversation.escrowId && (
                      <p className="text-xs text-[#bacac3] font-['JetBrains_Mono',monospace] uppercase tracking-tighter">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6 [&::-webkit-scrollbar]:hidden">
                {messages.length === 0 ? (
                    ...
                    return isOwn ? (
                      <div key={msg.id} className="flex flex-row-reverse items-start gap-3 max-w-[80%] ml-auto">
                        <div className="space-y-1 flex flex-col items-end">
                          ...
                          <span className="text-[10px] font-['JetBrains_Mono',monospace] text-[#bacac3] mr-1">{timeFormat(msg.createdAt)}</span>
                        </div>
                      </div>
                    ) : (
                      <div key={msg.id} className="flex items-start gap-3 max-w-[80%]">
                        ...
                        <div className="space-y-1">
                          ...
                          <span className="text-[10px] font-['JetBrains_Mono',monospace] text-[#bacac3] ml-1">{timeFormat(msg.createdAt)}</span>
                        </div>
                      </div>
                    );
                  })
              ...
              <footer className="p-6 bg-[#0b1325]/80 backdrop-blur-xl border-t border-[#3c4a45]/5 flex-shrink-0">
